...
def divide_test_set_reidentification(X_test: np.ndarray, y_test: np.ndarray, image_vol_names_test: np.ndarray, reidentification_results:str):
    """
    Divide the test set into three sets:
    """
    # import pdb; pdb.set_trace()
    mask_re = np.zeros(len(image_vol_names_test), dtype=bool)
    reidentification_results = pd.read_csv(reidentification_results)
    for idx, img_vol_name in enumerate(image_vol_names_test):
        img_vol_name = f"{img_vol_name}.nii.gz"
        img_vol_name_results = reidentification_results[reidentification_results['query'] == img_vol_name]
        subject_id_query, _ = get_subject_and_date(img_vol_name)
        subject_id_results, _ = get_subject_and_date(img_vol_name_results['result'].values[0])
        if subject_id_query == subject_id_results:
            mask_re[idx] = True
        else:
            mask_re[idx] = False

    test_re_success = { 'X': X_test[mask_re], 'y': y_test[mask_re], 'image_vol_names': image_vol_names_test[mask_re]}
    test_re_failure = { 'X': X_test[~mask_re], 'y': y_test[~mask_re], 'image_vol_names': image_vol_names_test[~mask_re]}

    print(f"Test set size: {len(X_test)}")
    print(f"Test set size for re-identification success: {len(test_re_success['X'])}")
    print(f"Test set size for re-identification failure: {len(test_re_failure['X'])}")
    return test_re_success, test_re_failure
...
def patient_level_three_way_split(X: np.ndarray, y: np.ndarray, patient_ids: np.ndarray, image_vol_names: np.ndarray,
    ...
    # 
    image_vol_names_test = np.array(image_vol_names)[test_mask]
    ...
    return X_train, X_val, X_test, y_train, y_val, y_test, patient_ids_train, patient_ids_val, patient_ids_test, image_vol_names_test
...
def prepare_features_Nyxus(features_dir: str, image_dir: str, info_csv: str,
                                   ...
                                   val_size: float = 0.2, classes: list[str] = ['M', 'F']):
    """
    ...
    for file_name in file_names:
        if file_name.endswith(".csv"):
            ...
            label = adni_info_df[(adni_info_df['Subject'] == subject_id)]['Sex'].values[0]
            if label in classes:
                labels.append(label)
                ...
                suffixes: str = "".join(Path(image_path).suffixes)
                stem: str = Path(image_path).name[: -len(suffixes)]
...
def prepare_features_BrainAIC(features_csv_path: str, info_csv: str, random_state: int = 42, test_size: float = 0.2, val_size: float = 0.2, 
    classes: list[str] = ['M', 'F']) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, list]:
    """
...
class ADNIClassifier(nn.Module):
    def __init__(self, input_size, hidden_size=128, dropout_rate=0.3, num_hidden_layers=1, num_classes=2):
        """
...
def print_averaged_classification_report(averaged_report, class_names, n_runs, re_identification: bool = False):
    """
    ...
    """
    print(f"\n{'='*80}")
    if re_identification:
        print(f"📊 AVERAGED CLASSIFICATION REPORT ACROSS {n_runs} RUNS FOR RE-IDENTIFICATION Success")
    else:
        print(f"📊 AVERAGED CLASSIFICATION REPORT ACROSS {n_runs} RUNS FOR RE-IDENTIFICATION Failure")
